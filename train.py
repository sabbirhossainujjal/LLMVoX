...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Training script with command-line config options')

    # System settings
    parser.add_argument('--device', type=str,
                        help='Device to use for training (e.g., cuda:0)')
    parser.add_argument(
        '--dtype', type=str, choices=['float16', 'bfloat16', 'float32'], help='Data type for training')
    ...
    parser.add_argument('--block_size', type=int,
                        help='Context size for attention')
    parser.add_argument('--dropout', type=float, help='Dropout probability')
    parser.add_argument(
        '--bias', type=bool, help='Whether to use bias in LayerNorm and Linear layers')
    ...
    parser.add_argument('--wav_config_path', type=str,
                        help='Path to wav tokenizer config')
    parser.add_argument('--wav_model_path', type=str,
                        help='Path to wav tokenizer model')
    ...
    parser.add_argument('--always_save_checkpoint', type=bool,
                        help='Whether to always save checkpoints')
    ...
    parser.add_argument('--decay_lr', type=bool,
...
def initialize_model(config, device):  # This is the function for initializing llm model
    ...
    if config['init_from'] == 'scratch':
        ...
        ckpt_path = os.path.join(
            config['out_dir'], config['checkpoint_filename'])
        ...
        print(f"Debugging: override_args: {override_args}")
        ...
        print(f"Debugging: model_args: {model_args}")
...
# This function sets up the phoneme tokenizer and encoder model
...
def main():
    ...
    model, model_args, iter_num = initialize_model(config, device)  # LLM model

    print(f"Debugging: Middle model {model}")
    ...
    print(f"Debugging: phoneme_model {llm_model}")
    # Create data loader
    ...
    X, Y = get_batch(train_dataloader, context_length,
                     device, device_type, llm_model)
    ...
    while True:
        ...
        lr = get_lr(
            config, iter_num) if config['decay_lr'] else config['learning_rate']
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

        # Evaluate and save checkpoint at specified intervals
        if iter_num % config['eval_interval'] == 0 and master_process:
            losses = estimate_loss(model, train_dataloader, context_length,
                                   config['eval_iters'], ctx, device, device_type, llm_model)
        ...
        for micro_step in range(env['gradient_accumulation_steps']):
            ...
            if ddp:
                model.require_backward_grad_sync = (
                    micro_step == env['gradient_accumulation_steps'] - 1)
        ...
        if iter_num % config['log_interval'] == 0 and master_process:
            ...
            if local_iter_num >= 5:  # Let training settle before measuring MFU
                mfu = raw_model.estimate_mfu(
                    config['batch_size'] * env['gradient_accumulation_steps'], dt)
        ...
        if iter_num > config['max_iters']:
            print(
                f"Reached maximum iterations ({config['max_iters']}), training complete")
            break
