...
def setup_environment(config):
    ...
    tokens_per_iter = gradient_accumulation_steps * \
        ddp_world_size * config['batch_size'] * config['block_size']
...
def setup_wandb(config, master_process):
    ...
    if config['wandb_log'] and master_process:
        try:
            ...
            print(
                f"Initializing wandb with project: {config['wandb_project']}, run name: {config['wandb_run_name']}")
...
def save_checkpoint(config, model, optimizer, iter_num, model_args):
    ...
    checkpoint_path = os.path.join(
        config['out_dir'], config.get('checkpoint_filename', 'ckpt.pt'))
