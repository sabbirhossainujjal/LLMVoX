...
class SpeechDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, device='cpu', 
        ...
        self.wavtokenizer = WavTokenizer.from_pretrained0802(wav_config_path, wav_model_path)
    ...
    def process_speech(self, speech_path):
        ...
        discrete_code = discrete_code[discrete_code != EOA_TOKEN_ID].unsqueeze(0).unsqueeze(0)
...
class SpeechDataCollator:
    ...
    def _process_speech_embeddings(self, instances, batch):
        ...
        speech_embeddings_list = [instance['speech_embeddings'] for instance in instances]
    ...
    def _process_speech_tokens(self, instances, batch):
        ...
        speech_tokens_list = [instance['speech_out_tokens'] for instance in instances]
...
def construct_speech_decoder_input(data, llm_model):
    ...
    combined_embeddings = torch.cat([text_label_embeddings, speech_embeddings], dim=2)
    
    # Normalize the combined embeddings
    combined_embeddings = F.normalize(combined_embeddings, p=2, dim=2, eps=1e-8)
...
def create_data_module(tokenizer, device, config=None):
    ...
    speech_folder_path=config.get('speech_data_folder', None) if config else None
